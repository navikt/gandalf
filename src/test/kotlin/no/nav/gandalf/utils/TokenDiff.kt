...
private fun diffAttributes(
    ...
    for (i in 0 until map1!!.length) {
        if (j < map2!!.length && map1.item(i).nodeName == map2.item(j).nodeName) {
            if (map1.item(i).textContent == null &&
                map2.item(j).textContent != null ||
                map1.item(i).textContent != null &&
                map1.item(i).textContent != map2.item(j).textContent
            ) {
                diff.add(
