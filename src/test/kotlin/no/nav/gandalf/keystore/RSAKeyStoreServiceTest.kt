...
class RSAKeyStoreServiceTest {
    ...
    fun `Get Current DBKey With Expired Key`() {
        ...
        rsaKeyStoreService.addRSAKey(LocalDateTime.now().minusSeconds(RSAKeyStoreRepositoryImpl.keyRotationTime + 10)) // expired, but not outdated
        Assert.assertTrue(rsaKeyStoreService.findAllOrdered().size == 1)
        val rsaKeyStore: RSAKeyStore? = rsaKeyStoreService.currentDBKeyUpdateIfNeeded // should add one new key to DB and the old key
        Assert.assertTrue(rsaKeyStore != null)
        Assert.assertTrue(!rsaKeyStore!!.hasExpired())
    ...
    fun `Get Current DBKey WithExpired Key And Outdated Keys`() {
        ...
        rsaKeyStoreService.addRSAKey(LocalDateTime.now().minusSeconds(2 * RSAKeyStoreRepositoryImpl.keyRotationTime + 10)) // expired and outdated (more than 2*keyRotationTime)
        rsaKeyStoreService.addRSAKey(LocalDateTime.now().minusSeconds(2 * RSAKeyStoreRepositoryImpl.keyRotationTime)) // expired and outdated
        rsaKeyStoreService.addRSAKey(LocalDateTime.now().minusSeconds(2 * RSAKeyStoreRepositoryImpl.keyRotationTime - 10)) // expired, but not outdated
        rsaKeyStoreService.addRSAKey(LocalDateTime.now().minusSeconds(2 * RSAKeyStoreRepositoryImpl.keyRotationTime + 10)) // expired, but not outdated
        rsaKeyStoreService.addRSAKey(LocalDateTime.now().minusSeconds(RSAKeyStoreRepositoryImpl.keyRotationTime + 10)) // expired, but not outdated
        println(rsaKeyStoreService.findAllOrdered().size)
        Assert.assertTrue(rsaKeyStoreService.findAllOrdered().size == 5)
        val rsaKeyStore: RSAKeyStore? = rsaKeyStoreService.currentDBKeyUpdateIfNeeded // should add one new key and delete the 2 outdated keys
        Assert.assertTrue(rsaKeyStore != null)
        Assert.assertTrue(!rsaKeyStore!!.hasExpired())
    ...
    fun `Get Current DBKey WithExpired Key And Outdated Keys Check MinNoofKeys`() {
        ...
        rsaKeyStoreService.addRSAKey(LocalDateTime.now().minusSeconds(2 * RSAKeyStoreRepositoryImpl.keyRotationTime + 20)) // expired and outdated (more than 2*keyRotationTime)
        rsaKeyStoreService.addRSAKey(LocalDateTime.now().minusSeconds(2 * RSAKeyStoreRepositoryImpl.keyRotationTime + 10)) // expired and outdated
        rsaKeyStoreService.addRSAKey(LocalDateTime.now().minusSeconds(2 * RSAKeyStoreRepositoryImpl.keyRotationTime + 5)) // expired and outdated
        Assert.assertTrue(rsaKeyStoreService.findAllOrdered().size == 3)
        val rsaKeyStore: RSAKeyStore? = rsaKeyStoreService.currentDBKeyUpdateIfNeeded // should add one new key and keep old keys according to minNoofKeys
        Assert.assertTrue(rsaKeyStore != null)
        Assert.assertTrue(!rsaKeyStore!!.hasExpired())
